...
use intl::unicode::{is_alphabetic, is_control, is_lowercase, is_uppercase, to_lowercase, to_uppercase};
...
fn predicates_match_std() {
    for cp in 0u32..=0x2FFF {
        ...
        assert_eq!(is_alphabetic(c), c.is_alphabetic(), "is_alphabetic U+{cp:04X}");
