...
// `kRSUnicode` order from `Unihan_kRSUnicode.txt`. Blob layout (little-endian):
// `[u32 count]`, then `count` sorted `u32` codepoints, then `count` `u16` packed
// keys, each `(radical*2 + is_simplified) << 7 | (residual + 16)`. Generated by
// codegen (`emit_collation_zh_rs`). See [`zh_rs_key`] and [`Tailoring::sort_key`].
#[cfg(feature = "collation-zh")]
    ...
    pub fn for_locale(lang: &str) -> Option<Tailoring> {
        ...
        if primary == "zh" {
            let table = match full.find("-u-co-").map(|i| &full[i + 6..]) {
                ...
                // `pinyin`, `standard`, an unknown/unsupported keyword (incl. the
                // deferred `unihan`), or none → the default pinyin table.
                _ => ZH_PINYIN,
            };
    ...
    pub fn sort_key(&self, s: &str) -> Vec<u16> {
        ...
        while i < cv.len() {
                ...
                && self.match_at(&cv[i..]).is_none()
            {
                ...
                if let Some(rank) = zh_ranked(table, cp) {
                    if !buf.is_empty() {
                    ...
                    // Unpack `(radical*2 + simplified) << 7 | (residual + 16)`; each
                    // field becomes its own ordering primary (both non-zero).
                    let radical_key = (packed >> 7) as u32;
                    let resid = (packed & 0x7F) as u32;
                    cea.push(pack(ZH_HAN_BASE, 0x0020, 0x0002));
                    cea.push(pack(ZH_RS_MARKER, 0x0000, 0x0000));
                    cea.push(pack(radical_key, 0x0000, 0x0000));
                    cea.push(pack(resid, 0x0000, 0x0000));
                    // Code point as the final tie-breaker (split across two 15-bit
                    // primaries, top-bit-set so both are non-zero and monotonic).
                    cea.push(pack(((cp >> 15) & 0x7FFF) | 0x8000, 0x0000, 0x0000));
                    cea.push(pack((cp & 0x7FFF) | 0x8000, 0x0000, 0x0000));
                    i += 1;
                    continue;
