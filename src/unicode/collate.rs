...
/// The pinyin primary rank of Han ideograph `cp` under CLDR `zh` collation, or
/// `None` if `cp` is not a listed ideograph (then DUCET radical-stroke order is
/// the fallback). Binary-searches the codepoint array of [`ZH_PINYIN`]; no alloc.
#[cfg(feature = "collation-zh")]
fn zh_pinyin_rank(cp: u32) -> Option<u16> {
    let count =
        u32::from_le_bytes([ZH_PINYIN[0], ZH_PINYIN[1], ZH_PINYIN[2], ZH_PINYIN[3]]) as usize;
    ...
    while lo < hi {
        ...
        match v.cmp(&cp) {
            ...
            Ordering::Equal => {
                let r = ranks + mid * 2;
                return Some(u16::from_le_bytes([ZH_PINYIN[r], ZH_PINYIN[r + 1]]));
            }
        }
    }
    None
...
impl Tailoring {
    ...
    pub fn for_locale(lang: &str) -> Option<Tailoring> {
        ...
        // Chinese defaults to pinyin collation (feature `collation-zh`). Handled
        // before the CLDR rule table since zh's pinyin rule is a per-codepoint Han
        // weight table, not a runtime rule string.
        #[cfg(feature = "collation-zh")]
        if primary == "zh" {
            return Some(Tailoring::zh_pinyin());
        }
    ...
    pub fn sort_key(&self, s: &str) -> Vec<u16> {
        ...
        let mut i = 0;
        while i < cv.len() {
            // zh pinyin: a Han ideograph with a pinyin rank is weighted by that
            // rank (base `ZH_HAN_BASE`, rank in the tailoring sub-weight) instead
            // of its DUCET implicit weight. Tailoring entries (tone vowels) are
            // matched first; non-Han and unlisted ideographs fall through to the
            // buffered DUCET path.
            #[cfg(feature = "collation-zh")]
            if self.han_pinyin && self.match_at(&cv[i..]).is_none() {
                let cp = cv[i] as u32;
                ...
                //   • a listed ideograph → its pinyin rank (matches ICU/V8);
                //   • an unlisted ideograph → its DUCET implicit weights, whose
                //     `AAAA` (≈0xFB40) exceeds every rank, so all unlisted Han sort
                //     *after* every pinyin char (as ICU does) yet still before
                //     Latin, ordered among themselves by code point (a proxy for
                //     ICU's radical-stroke fallback).
                // Non-Han falls through to the buffered root (DUCET) path.
                if let Some(rank) = zh_pinyin_rank(cp) {
                    if !buf.is_empty() {
    ...
    /// The Chinese (`zh`) **pinyin** collator: the default `Intl.Collator('zh')`
    /// order. Han ideographs are weighted by their CLDR pinyin rank (pinyin
    /// syllable → tone → stroke), placed — per `[reorder Hani]` — after digits
    /// and before Latin and other scripts; non-Han uses the root (DUCET) order.
    /// Ideographs absent from the CLDR rule keep their DUCET radical-stroke order.
    ///
